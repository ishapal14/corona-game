...
function draw() 
{
  ...
  for (var i = 0; i < virusGroup.length; i++) {
    ...
    for (var j = 0; j < humanGroup.length; j++) {
      if (virusGroup[i] !== undefined && humanGroup[j] !== undefined) {
        var collision = Matter.SAT.collides(virusGroup[i].body, humanGroup[j].body);
        if (collision.collided) {
          humanGroup[j].remove(j);
  
          Matter.World.remove(world, virusGroup[i].body);
...
function showHuman() {
  if(frameCount%300===0){
   ...
   var human= new Human(200,random(500,600),50,50);
   humanGroup.push(human);
  }

 for (var i = 0; i < humanGroup.length; i++) {
   Matter.Body.setVelocity(humanGroup[i].body,{x:0.9,y:-0.2} );

   humanGroup[i].display();
 }
}
...
function newVirus() {
  if(frameCount%300===0 && virusGroup.length<10){
   ...
   var virus= new Virus(random(1500,1600),random(100,200),50,50);
   virusGroup.push(virus);
  }
